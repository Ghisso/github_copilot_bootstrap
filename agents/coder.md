...
**Control-plane route:** If any target file is a bootstrap control-plane file — `.claude/hooks/`, `.claude/settings.json`, `.claude/hooks/`, `.codex/`, `CLAUDE.md`, `AGENTS.md`, `.mcp.json`, or `.devcontainer/` — treat as high-risk. Pause and ask the user to confirm the change before applying. These files affect every session in this project.
