...
This repository is not an application. It is a source-of-truth plus generated bootstrap for AI coding agents. Maintainers edit `shared/`, a generator renders it into one installable tree, an installer copies that tree into a consumer, and the consumer's `.claude/` becomes its own Git repository for AI state.
...
flowchart LR
    ...
    D -->|install_bootstrap.py| C[consumer root adapters]
    D -->|install_bootstrap.py| N[nested .claude repo]
    N -->|state-sync.sh| R[(ai-state branch)]
```

| Place | What it holds | Who edits it |
| --- | --- | --- |
| `shared/` here | policies, agent metadata and prompts, skills, hooks, MCP definitions, templates, the verifier, memory seed, devcontainer | bootstrap maintainers, by hand |
| `dist/multi-agent/` here | the single installable target rendered from `shared/` | nobody; regenerated by the generator and git-ignored |
| a consumer's outer repository | thin root adapters (`AGENTS.md`, `CLAUDE.md`, `.mcp.json`, `.github/**`, `.codex/**`, `.agents/**`, `.vscode/**`) plus the installed `.claude/` basis | the installer writes adapters; the consumer edits only project context and its own state |
| the nested `.claude/` repository | a separate Git repository on branch `ai-state` with the installed basis and the mutable AI state | hooks and the installer commit it; the outer repository ignores it |

The authoring repository's root `.gitignore` states both boundaries: `dist/` is generated build output, and `.claude/` is tracked in the nested repository, not the outer one.
...
`scripts/generate_targets.py` has exactly one target, `multi-agent`. Its `generate` function resets the output directory, calls `render_multi_agent`, and strips quarantined content. `render_multi_agent` runs a fixed sequence:
...
- skills and paths, then docs parity (which owns the skill-library integrity rules);
