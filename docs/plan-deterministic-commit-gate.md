...
- **Change:** in [validate_targets.py](../scripts/validate_targets.py), add throwaway-repo cases that set `core.hooksPath` to the generated dir and run real `git commit`s. All commits below are on a `<plan>_implementation` branch unless stated:
  - invalid state (no findings report / an open CRITICAL or MAJOR finding / a surviving MINOR finding without an explicit disposition and reason / stale `content_hash` / small-plan not `complete` or valid `paused` / current phase is `cancelled` / missing closeout `**Status:** COMPLETED` / missing `## [LEARN] Entries` evidence) → **blocked** (`git commit` exits non-zero). A valid paused current small plan may create a checkpoint commit without final findings/LEARN/DOCUMENT/COMPLETED closeout; it does not advance the phase and still blocks push/PR closeout. A cancelled current phase gets a distinct stale-pointer message because cancellation never certifies a commit.
