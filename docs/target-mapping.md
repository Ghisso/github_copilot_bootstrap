...
Structural validation checks exact normalized prompt-body parity for all six roles and records their actual `developer_instructions` sizes (currently 3,145–8,202 bytes). The current official custom-agent schema does not publish a separate `developer_instructions` size cap; these measurements are observability, not a product limit. Phase I remains the delivery check: trusted native Codex probes must confirm all six roles receive the complete instructions without truncation on two supported client versions. Claude's native `.claude/agents/` behavior is unchanged.
