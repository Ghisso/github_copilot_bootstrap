...
4. **Closeout dry run.** `git add` the intended outer files (never `git add -A`; never `openwiki/.run.json`), inspect `git diff --cached`, then run `uv run python .claude/scripts/verify.py closeout --format text` **without** `--persist`. It runs the plan's required `## Verification` items and prints one summary line per item. Paste those lines into the session log's `## Verification` section. If any item is not `PASS`, return to IMPLEMENT. *Why before the second checkpoint:* the closeout receipt hashes the session log, so the log must be final before the receipt exists.
5. **Second nested checkpoint.** Same command as step 3. This is the last nested checkpoint before the commit; the receipts in step 7 bind the big plan bytes it records.
6. **Findings.** Give every surviving MINOR an explicit `disposition` and non-empty `reason`, then persist with one `--profile <name>` per reviewed profile: `record_findings.py . --findings-json - --phase <current_phase> --profile ... --out .claude/quality_reports/findings-<current_phase>.json`. *Why after staging:* the report records `dirty: true` when any tracked change is unstaged, and the commit gate requires `dirty: false`.
7. **Receipts.** `uv run python .claude/scripts/verify.py phase --format json --persist`, then `uv run python .claude/scripts/verify.py closeout --format json --persist` (add `--documentation-na "<reason>"` only when documentation is explicitly not applicable). *Why after staging:* the receipt's `tree_sha` is `git write-tree` on the index; a receipt made before staging is rejected with `closeout receipt final tracked state is stale`. Closeout also needs the findings report and the phase receipt from steps 6 and 7 to exist, or it fails.
8. **Commit.** `git commit` in its **own** Bash command, after steps 6 and 7 have finished. The `post-commit` hook checkpoints and publishes nested state; do not do that yourself.
9. **Push.** One normal push to the branch upstream with `GIT_TERMINAL_PROMPT=0`; a missing remote or an authentication failure is a warning, not a failure.

Two hard rules:

- **Never put `git commit` in the same Bash command as steps 6 or 7.** The commit gate hook inspects the *text* of every Bash command before it runs; a chain such as `record_findings ... && verify.py closeout --persist && git commit` is judged on the receipts that exist *before* the chain runs, is denied in full, and none of the steps execute.
- **Never checkpoint, publish, or push nested state between step 7 and step 8.** That changes what the receipts bind to and fails the commit with `closeout receipt governing control-plane provenance is stale`. Findings and receipt files are not provenance-bound, so writing them after step 5 is expected.

The reviewer does not persist findings; the coder does not create receipts; the orchestrator does steps 2 to 9 itself. An open CRITICAL or MAJOR finding blocks step 8; resolve it in the FIX LOOP rather than deferring it to push or PR.
