...
**Control-plane route:** If any target file is under `.claude/agents/`, `.claude/skills/`, `.claude/hooks/`, or is `CLAUDE.md` or `.claude/settings.json` — treat as high-risk. Pause and ask the user to confirm the change before applying. These files affect every session in every project that uses this bootstrap.
