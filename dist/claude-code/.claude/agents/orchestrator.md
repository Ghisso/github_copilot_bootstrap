...
4. **Routing decision:** Based on exploration, classify the task and pass the decision to `planner`:
   ...
   - **Always full-plan if any control-plane file is touched** (`.claude/agents/**`, `.claude/skills/**`, `.claude/hooks/**`, `.claude/settings.json`, `CLAUDE.md`)
...
**Complexity gate:**
- **Control-plane files** (`.claude/agents/**`, `.claude/skills/**`, `.claude/hooks/**`, `.claude/settings.json`, `CLAUDE.md`): always non-trivial, always run full reviewer set regardless of diff size
