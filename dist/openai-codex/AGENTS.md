...
Custom agents are rendered as Codex project custom agents in `.codex/agents/*.toml`. Copilot and Claude model pins are intentionally omitted; review helpers use Codex-native primary/adversarial agents.
...
1. Check `.codex/MEMORY.md` for relevant `[LEARN]` entries
2. For ambiguous/complex tasks: clarify with user (max 3-5 questions), optionally create a spec in `.codex/quality_reports/specs/`
3. Draft plan → save to `.codex/plans/YYYY-MM-DD_short-description.md` (for concrete implementation plans) or `.codex/explorations/YYYY-MM-DD_description/` (for exploratory/PoC plans)
...
**SCORE:** See the Quality Gates & Testing Protocol section below.
- Score ≥ 80 = commit
- Score ≥ 90 = PR-ready
...
**Log location:** `.codex/session_logs/YYYY-MM-DD_description.md`
...
Merge-time review reports should be stored in `.codex/quality_reports/merges/`.
...
1. Save `[LEARN]` entries to `.codex/MEMORY.md`
2. Update session log
3. Ensure plan is saved to disk
4. Document open questions

**Starting a new session:**
1. Read `AGENTS.md` + most recent plan in `.codex/plans/` or exploration in `.codex/explorations/`
...
```
[ ] Plan saved to .codex/plans/ or .codex/explorations/
...
These protections are enforced by hooks in `.codex/hooks.json`.
...
- Session start/end events are logged to `.codex/session_logs/hooks-sessions.log`
- Runtime hook errors are logged to `.codex/session_logs/hooks-errors.log`
...
```
→ Update session log in .codex/session_logs/
→ Flush [LEARN] entries to .codex/MEMORY.md
```

**When the conversation is long / context is filling up:**
```
→ Save plan to .codex/plans/ (implementation) or .codex/explorations/ (research/PoC)
...
Follow the testing guidance in this file for detailed mocking rules.
