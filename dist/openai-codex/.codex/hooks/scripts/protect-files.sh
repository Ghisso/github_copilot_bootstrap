...
# Compute roots from the generated target namespace:
# hooks/scripts/ -> hooks/ -> .github|.claude|.codex -> repo
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
TARGET_CONFIG_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../../.." && pwd)"
HOOK_NAMESPACE="$(basename "$TARGET_CONFIG_ROOT")"
export REPO_ROOT
export HOOK_NAMESPACE

INPUT=$(cat)
OUTPUT=$(printf '%s' "$INPUT" | python3 -c 'import json, os, posixpath, re, shlex, sys

repo_root = os.environ.get("REPO_ROOT", "").rstrip("/")
hook_namespace = os.environ.get("HOOK_NAMESPACE", "")
...
if hook_paths:
  ...
  decision = "deny" if hook_namespace == ".codex" else "ask"
  reason = (
