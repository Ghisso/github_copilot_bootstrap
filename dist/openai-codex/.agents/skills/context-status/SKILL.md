...
### 1. Active Plan
```bash
ls -lt .codex/plans/*.md 2>/dev/null | grep -v ".gitkeep" | head -3
```

### 2. Session Log Recency
```bash
ls -lt .codex/session_logs/*.md 2>/dev/null | grep -v ".gitkeep" | head -3
```

### 3. MEMORY.md
```bash
wc -l .codex/MEMORY.md
grep -c "\[LEARN" .codex/MEMORY.md
```
