...
description: |
  ...
  retrieval; groundedness, completeness, and hallucination rate for generation.
  Diagnoses failure root causes and recommends improvements. Use when:
  - "audit RAG pipeline", "RAG quality", "evaluate RAG retrieval"
...
- A set of test queries with known-correct answers (or willingness to design them)
...
$$\text{Recall@K} = \frac{\text{relevant chunks in top-K}}{\text{total relevant chunks in corpus}}$$
...
Normalized by dividing by the ideal DCG (chunks sorted by relevance).
