...
description: |
  ...
  and vertical scroll (keyboard/scroll). Multiple visual themes. Use when:
  - "create a presentation", "make slides", "build a deck"
  - "convert to slides", "HTML presentation", "web-based slides"
  - "reveal.js deck", "browser presentation"
  - Converting a document, markdown, outline, or notes into presentable slides
  Do NOT use for PowerPoint/PPTX output.
---
...
2. **CDN dependencies** (horizontal mode):
   ...
   - `https://cdn.jsdelivr.net/npm/reveal.js@4.5.0/plugin/highlight/highlight.js` (if code blocks)
   - `https://cdn.jsdelivr.net/npm/reveal.js@4.5.0/plugin/highlight/monokai.css` (if code blocks)
...
Define theme as CSS custom properties for easy switching:
...
| Theme | Background | Accent | Heading Font | Body Font |
|---|---|---|---|---|
| `dark-editorial` | #0a0a0a | #ff6b35 | Playfair Display / Georgia | Inter / system-ui |
| `light-minimal` | #ffffff | #2563eb | system-ui | system-ui |
| `corporate` | #1e293b | #3b82f6 | system-ui | system-ui |
| `hacker` | #0d1117 | #39d353 | 'Courier New', monospace | 'Courier New', monospace |
...
| Problem | Fix |
|---|---|
| CDN fails to load (Reveal.js, Lucide, Google Fonts) | Switch to inline styles/scripts: embed Reveal.js core inline, replace Lucide icons with inline SVG paths, use system font stack |
| Content overflows slide viewport | Split the slide into two: move supporting detail to a follow-up slide or convert prose to a bullet list |
| Fonts render incorrectly or fail to load | Add system font fallback in the font-family stack |
| Reveal.js fails to initialize | Verify CDN script tags appear before `Reveal.initialize()`; check browser console for 404s |
...
- [ ] No more than 2 consecutive slides use the same layout pattern
