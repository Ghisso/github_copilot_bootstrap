...
matching `phase_checks`' authoring branch in `shared/scripts/verify.py`. Its
own tooling comes from the devcontainer image (`.devcontainer/Dockerfile`,
`uv pip install --system`), not `uv sync` — `.devcontainer/devcontainer.json`'s
`postCreateCommand` runs `uv sync` only when a `pyproject.toml` is present,
which is true for a consumer project generated by this bootstrap, not for
