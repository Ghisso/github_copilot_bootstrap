...
## 9. Post-implementation review & remediation (2026-07-08T14:02Z)

Reviewed the implemented branch `git-state-sync` (commits `R-SYNC-05a`..`R-SYNC-05f`). The core is sound: `generate_targets.py --all && validate_targets.py` is green, the Phase 6 `validate_state_sync` suite genuinely exercises two-machine divergence / same-line conflict / the stdin contract / `--state-remote` against real bare remotes, the Phase 5 grep sweep is clean, `dist/` is drift-free, both new scripts pass `bash -n`, all doc links resolve, and `plan-post-review-hardening.md` is byte-identical. The review surfaced four items. Each is fixed below in the one-commit-per-phase style (`R-SYNC-05g`+); every phase leaves regenerate + validate green and `dist/` drift-free.

### Findings

- **F1 [Medium] — `restore-root-adapters.sh` is never invoked by `state-sync.sh setup`.** D5 says the restore helper is "called by `state-sync.sh setup` and `post-start.sh`", but only `post-start.sh` calls it (`cmd_setup` in `state-sync.sh` has no reference to it). Consequence: outside the devcontainer, root adapters (`CLAUDE.md`, `AGENTS.md`, `.mcp.json`, `.codex/**`, `.vscode/*.json`, the Copilot surface when not committed) are **not** restored when a non-devcontainer machine first materialises `.claude/`, because `SessionStart`/folderOpen run only `pull` (which reaches `cmd_setup`, but that never restores). The Phase 6 suite never exercised the restore path, so this passed CI.
- **F2 [Low] — the 2-second stdin drain now runs on installer/updater-invoked `setup`/`push`/`migrate-from-hf`.** The drain at the top of `state-sync.sh` was written for hook/task invocation. `install_bootstrap.py` and `update_consumers.py` invoke `state-sync.sh` via `subprocess.run(...)` with inherited stdin, so an **interactive** install/update blocks on (and silently swallows) up to ~2s of terminal input per call (~4s for setup+push). Not a hang (the drain is bounded), but a behaviour change from the pre-git installer path, which never drained.
- **F3 [Low] — pull/push timeout asymmetry.** The Stop `push` hook gets `timeout: 180`, but the `SessionStart`/folderOpen `state-sync.sh pull` gets `timeout: 10` even though `git pull --rebase` is equally network-bound. A slow pull is SIGKILLed mid-rebase; it self-heals on the next run (`git rebase --abort` clears the leftover state, with a `WARN`) but the pull is skipped and the tree is briefly left mid-rebase.
- **F4 [Low] — `--autostash` pop can leave a half-merged tree, contra D4.** D4 promises local state is "never destroyed or half-merged by automation". In the *standalone* `pull` path, uncommitted local edits that collide on the same lines as incoming changes make `git pull --rebase --autostash` complete the rebase and then fail the stash *pop*, leaving conflict markers in the working tree; the error path's `git rebase --abort` is a no-op there (the rebase already finished), so the contract is not upheld. Data is not lost (the autostash entry is retained), and the risk is low because `.claude/` is normally clean at `SessionStart` (the prior Stop hook committed everything), but it is a genuine gap in the D4 primitive as invoked standalone.

### Phase 7 — `R-SYNC-05g`: restore root adapters on `setup` (F1)

- [shared/hooks/scripts/state-sync.sh](../shared/hooks/scripts/state-sync.sh): at the end of `cmd_setup` (fresh-init path only — `cmd_setup` already early-returns when `.claude/.git` exists), invoke the co-located `restore-root-adapters.sh` (guard for its presence; warn-never-fail), so **every** entry point that first materialises `.claude/` — not just the devcontainer `post-start.sh` — restores the root adapters D5 carries in `bootstrap-root/`. Post-start.sh keeps its own explicit `restore` call (it runs after `pull`, covering a `pull` that brings a newer `bootstrap-root/`); the extra call in `cmd_setup` is idempotent (`cp` of identical bytes) and harmless.
- [scripts/validate_targets.py](../scripts/validate_targets.py): extend `validate_state_sync` — on "machine B" (a fresh clone with no `post-start.sh`), after `setup`, assert a root adapter (`CLAUDE.md`) exists at the repo root, restored from `bootstrap-root/`. This is the coverage gap that let F1 pass.

**Acceptance:** the new machine-B assertion passes; regenerate + validate green; `dist/` drift-free.

### Phase 8 — `R-SYNC-05h`: sync-invocation robustness (F2, F3, F4)

- **F2** — [scripts/install_bootstrap.py](../scripts/install_bootstrap.py), [scripts/update_consumers.py](../scripts/update_consumers.py): pass `stdin=subprocess.DEVNULL` to every `state-sync.sh` subprocess invocation, so an interactive install/update neither blocks on nor swallows terminal input via the drain. (`validate_targets.py`'s `run_state_sync` gets the same treatment for determinism; the Phase 6 stdin-contract case keeps its own `Popen(stdin=PIPE)` and is unaffected.)
- **F3** — [shared/hooks/hooks.json](../shared/hooks/hooks.json), [scripts/generate_targets.py](../scripts/generate_targets.py): raise the `SessionStart` `state-sync.sh pull` timeout from 10s to 60s (still well under the Stop `push` 180s), matching its network cost. The validator only asserts the timeout keys exist, not their values, so no assertion change is needed.
- **F4** — [shared/hooks/scripts/state-sync.sh](../shared/hooks/scripts/state-sync.sh): extract the "commit dirty local state as a `session:` snapshot" step `cmd_push` already performs into a shared `commit_local_state` helper, and call it in `cmd_pull` *before* the rebase. A clean tree before `git pull --rebase` means the only reachable failure is a rebase conflict — which is aborted cleanly — never an `--autostash` pop conflict that leaves the tree half-merged. The clean-tree case is unchanged (nothing to commit → no-op), and `cmd_pull` called from within `cmd_push` is a no-op there too (push already committed). `--autostash` is retained as belt-and-suspenders.

**Acceptance:** regenerate + validate green including the Phase 6/7 suite; `dist/` drift-free; `plan-post-review-hardening.md` still byte-identical.
