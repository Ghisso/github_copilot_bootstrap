...
- [ ] `2026-09-19_phase-F3-hook-target-scoping` — scope two guards to the target a command acts on rather than to the command's text or the hook's own install location: `enforce-commit-gate.sh` stops judging commits made in other repositories, and `protect-files.py` stops matching the credential-file pattern inside longer words and stops treating this repository's control-plane filenames as protected in any directory. Credential-shaped names stay protected everywhere.
...
| F3 | coder + documenter | `shared/hooks/scripts/_lib-frontmatter.sh`, `shared/hooks/scripts/reporting-reminder.sh`, `shared/hooks/scripts/enforce-commit-gate.sh`, `shared/hooks/scripts/protect-files.py`, `tests/test_hook_gates.py`, `tests/test_lifecycle_hooks.py`, `docs/runtime-checks.md`, `docs/smoke-tests.md`, `docs/architecture.md` | `ponytail` (full), `refactor`, `code-style`, `testing-patterns`, `documentation`, `humanize` | `code`, `architecture`, `security`, `tests`, `ponytail` | focused hook-gate/classifier tests against real temporary repositories; generate/validate/check_runtime; `verify.py fast`; receipts |
