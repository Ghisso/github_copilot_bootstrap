...
- UserPromptSubmit (Codex)
  - Runs `state-sync.sh push` as a 60-second retry, preserving the checkpoint-then-publish contract between turns.
- SessionEnd (Codex)
  - Runs only `state-sync.sh checkpoint` with a three-second limit. It is delayed and best-effort, performs no publication, and is not a replacement for post-commit or the manual **AI state: push** task.
