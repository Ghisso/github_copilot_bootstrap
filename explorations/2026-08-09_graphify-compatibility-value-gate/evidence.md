...
directory. These are recorded limitations, not waived checks.
...
repository runs used `--out /tmp/graphify-phase0.Gg5y04/...`; temporary
...
| Incremental extract | same extract argv against an unchanged output directory | reports `0 code ... changed; 49 unchanged; 0 deleted` and `outputs left untouched`; exit 0 |
...
| Missing graph | query with a non-existent `--graph` path | stderr `error: graph file not found: ...`, exit 1 |
...
Control command (exit 1 as expected):

```text
docker run --rm --network none ... python3.13 \
  -c "import socket; socket.create_connection(('1.1.1.1', 443), 2)"
```
...
Graphify and the fixture were read-only. No credentials, LLM, API client, or
source-network access was requested. This mandatory privacy check passes.
...
source reads. Graphify used:

```text
graphify extract /home/ghisso/work/github_copilot_bootstrap \
  --code-only --no-cluster --out /tmp/graphify-phase0.Gg5y04/repo-warm
graphify query "<required question>" \
  --graph /tmp/graphify-phase0.Gg5y04/repo-warm/graphify-out/graph.json
```
...
| No-op semantic/raw churn | graph SHA-256 `af2d8c9d144677421aa7401193eb17488ed9f41896caffeb20b8ff522ffd9d11` before and after each no-op; 0-byte graph change | 0% | Pass |
...
source and received only three docstring mutations. This is a measurement
limitation imposed by the Phase 0 no-workspace-write boundary, not a reason to
relax the gate.
...
| Code-only network denial | Pass | Docker `--network none` control and extraction/update/query operations |
...
| Cold/warm/no-op/size/churn budgets | Pass with external-output update limitation | timings and SHA above |
...
Any one mandatory failure is NO-GO. This experiment has three: insufficient
real-question value, a known material false lead, and an incomplete
commit-backed branch-switch test. The privacy pass and performance passes do
not override them.
...
The outer repository had no tracked, staged, or untracked Phase 0 file change
before this ignored evidence artifact was created. Existing nested `.claude`
changes (`plans/graphify-structural-code-intelligence.md`,
`session_logs/hooks-errors.log`, and `graphify-out/cache/stat-index.json`) were
present outside this task’s write boundary and were not changed or cleaned by
this experiment. Final outer and nested status must be rechecked after the
orchestrator records its lifecycle evidence.
