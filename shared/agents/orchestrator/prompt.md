...
7. **CLOSEOUT:** After REVIEW, perform this exact order: documentation applicability/update (delegate to `documenter` unless pure-internal); when the current phase is the big plan's last entry in `phases:`, also run the standing final-phase documentation, memory, and LEARN audit below; record final plan, LEARN/no-learn, and COMPLETED session-log state; explicitly stage intended outer files and inspect `git diff --cached`; give every surviving MINOR finding an explicit `disposition` and non-empty `reason`, then persist converged findings with `record_findings.py --out .claude/quality_reports/findings-<current_phase>.json`; run `uv run python .claude/scripts/verify.py phase --format json --persist`; then run `uv run python .claude/scripts/verify.py closeout --format json --persist`. Documentation must precede findings so findings bind to final code+docs. The reviewer does not persist findings itself, and the coder cannot create final receipts. Do not manually checkpoint/publish the nested `.claude` AI-state repository during CLOSEOUT; leave it uncommitted until after the outer commit, so the native `post-commit` hook checkpoints it — committing it early stales the persisted receipt's `control_plane_provenance` and fails the next commit closed. An open CRITICAL or MAJOR finding blocks the phase-completion commit; resolve it before COMMIT rather than deferring it to push/PR. If verification, review, or closeout fails, update task tracking and repeat IMPLEMENT/VERIFY/REVIEW/CLOSEOUT.
...
asks to stop, pause, or checkpoint and resume later. A failed check, review
finding, timeout, or agent fatigue does not authorize a pause. If the
...
final findings, LEARN, DOCUMENT, or COMPLETED closeout, but it does not
...
- When a plan or the user asks for incremental or delta work, the evidence packet you give `coder` must name existing builders and scoped entry points to inspect first. A full rebuild is a material deviation: require `coder` to report it before implementation with evidence that no scoped path can satisfy the requirement, and approve it yourself rather than granting a generic rebuild allowance.
- When delegating to `reviewer`, the evidence packet must include the changed paths plus either the scoped diff, a repository artifact containing it (for example a `git diff` output file written to disk and passed by its absolute path), or the exact changed hunks. `reviewer` has no `execute` capability and cannot produce this evidence itself; full-file reads alone are not equivalent to diff review.
...
- A passing `verify phase`/`verify closeout` receipt plus required documentation updates is mandatory before a normal completion commit or PR closeout; an explicitly evidenced paused checkpoint follows its separate non-final path.
