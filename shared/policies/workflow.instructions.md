...
- Session start pulls mutable AI state on the git-backed `ai-state` branch (`.claude/` is its own nested git repo; see `state-sync.sh`). Codex Stop is turn-scoped: its one sequential wrapper logs, checks, checkpoints, then best-effort publishes while returning JSON-only stdout. Codex `UserPromptSubmit` retries compatible `push` (60 seconds); delayed best-effort `SessionEnd` checkpoints locally only (3 seconds). Closing a browser or editor tab is not a guaranteed lifecycle event, so do not rely on it for durability. The durable checkpoint-and-publish paths remain the `post-commit` git hook (after every outer-repo commit) and the explicit "AI state: push" VS Code task (manual, for state between commits).
