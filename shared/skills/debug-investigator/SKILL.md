...
description: |
  ...
  instrumentation point planning, and minimal reproduction design. Use when:
  - "debug this systematically", "root cause analysis"
  - "bisect this bug", "rank hypotheses for this error"
  - "help me isolate this issue", "create a minimal reproduction"
  - "instrumentation plan for this bug", "why does this keep failing"
  Use for non-obvious bugs that need systematic investigation, not simple
  errors the model diagnoses directly.
---
...
1. **Stacktrace interpretation** — If a traceback exists, read it bottom-up. The last frame is where the error manifested, but the cause is often several frames up. Identify:
   ...
   - Any familiar patterns (common error signatures below)
...
|---|---|---|
| `KeyError` / `AttributeError` | Missing key/attr, wrong type | Check data flow into the failing function |
| `TypeError: NoneType` | Unexpected None return value | Trace the function that should have returned a value |
| `ImportError` / `ModuleNotFoundError` | Missing or conflicting dependency | Check venv, installed packages, sys.path |
| `ConnectionError` / `TimeoutError` | External service unreachable | Check service status, network, credentials |
| `RecursionError` | Unbounded recursion or circular reference | Check call chain for cycles |
| `PermissionError` | File/directory access rights | Check file ownership and permissions |
...
2. **Bisection** — If the bug appeared after a change and H1 fails:
   ...
   - Binary search narrows to the causal commit in $\log_2(n)$ steps
...
5. **Reproduce before fixing.** If you cannot reproduce the bug in a controlled environment, any fix is speculative. Invest in reproduction first.
...
- The "bug" is actually a feature request or design disagreement — redirect to discussion
